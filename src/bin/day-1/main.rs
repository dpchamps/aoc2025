...
impl Direction {
    fn parse(input: Vec<String>) -> Result<Vec<Direction>>{
        Ok(input.iter().map(|line| {
            ...
            match direction {
                "L" => Ok(Direction::Left(count.parse().map_err(|_| anyhow!("Parse Error"))?)),
                "R" => Ok(Direction::Right(count.parse().map_err(|_| anyhow!("Parse Error"))?)),
    ...
    pub fn rotate(&mut self, direction: &Direction) {
        let result = match direction {
            Direction::Left(count) => {
                self.dial_state as isize - *count as isize
            },
